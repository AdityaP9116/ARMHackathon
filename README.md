...
| [silvermpx/mamba-rs](https://github.com/silvermpx/mamba-rs) | Standalone Rust runtime covering **Mamba-3 SISO**, CPU with rayon + optional BLAS, CUDA via NVRTC | A full-model runtime with **no framework dependency** — a library and CLI, not a PyTorch-callable op; its optimization story is x86/CUDA, not Arm/NEON; 1D only, no SS2D. **Listed because it is the closest thing to this project that exists**, and because a Mamba-3 CPU kernel would have to be claimed narrowly against it |
| [rishikksh20/mamba3-pytorch](https://github.com/rishikksh20/mamba3-pytorch) | Pure-PyTorch Mamba-3 (trapezoidal + RoPE on B/C, SISO and MIMO) | Explicitly educational — "no Triton/CUDA kernels… designed for understanding the algorithm." No SIMD, no Arm tuning, architecture only (no weights). Useful as a **correctness oracle**, not as a fast path |
