...
| `golden/mamba3/*.npz` | Ground truth **captured from the official GPU kernels** (Stage 0), not computed here: 10 cases across 7 shapes from the real `mamba3-siso-187m` checkpoint plus an edge sweep. Inputs are stored **post-cast**, since the kernel downcasts `Q/K/V/Trap/Angles/Z` to bf16 on entry. Replays with numpy alone — no torch, no CUDA. |
| `verify_golden_mamba3.py` | Reference vs those goldens. Tolerance is bf16 ULPs at tensor scale, **not** 1e-4: the kernel emits bf16, so 1e-4 is unsatisfiable. See the file header before changing the bound. |
| `check_mamba3_op.py` | The torch op end to end — angle pre-pass, head-major→time-major permute, pointer marshalling, and `torch.compile` composability. None of those are covered by the Rust or FFI tests, and each can produce plausible numbers when wrong. |
